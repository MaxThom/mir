<script lang="ts">
	...
	let filtered = $derived.by(() => {
		...
		return devices.filter((d) => {
			...
			const labels = Object.entries(d.meta?.labels ?? {})
				.map(([k, v]) => `${k}=${v}`)
				.join(' ')
				.toLowerCase();
						...
						<Table.Row class="hover:bg-transparent">
							<Table.Cell colspan={7} class="py-8 text-center">
								<div class="flex flex-col items-center gap-2 text-muted-foreground">
									...
									<span class="text-sm"
										>{search ? 'No devices match your search.' : 'No devices found.'}</span
									>
								</div>
							</Table.Cell>
						</Table.Row>
					{:else}
						{#each filtered as device (device.spec?.deviceId ?? device.meta?.name)}
							...
							>
								...
								<Table.Cell class="text-sm font-medium">{device.meta?.name ?? '—'}</Table.Cell>
								<Table.Cell>
